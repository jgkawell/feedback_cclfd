...
class ProcessInput:
    ...
    def getSimScores(self, word):
        ...
        if word in self.exactDict:
            ...
            for k in self.matchDict:
                ...
                scores = [syn.wup_similarity(comp) for syn in wordnet.synsets(word) if syn.wup_similarity(comp) is not None]
    ...
    def processUserInput(self, string):
        ...
        words = [word for word in string.split() if word not in self.stop_words]
        sims = {}
...
if __name__ == "__main__":
    ...
    pi.buildDicts()
    print(pi.processUserInput('i didn\'t like that the robot moved the cup over the laptop'))